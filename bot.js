...
**!смс [id] [сообщение]** - отправляет сообщение в вк указанному id (для бесед id отрицательный, например **-1**);
**!переписка** - выводит 10 последних сообщений из вк (сколько влезит, если длинные);
***\`\`\`P.S. Создатель бота daniil#4337, все вопросы, пожелания и предложения в лс.\`\`\`***`), mess.channel.guild.id);
