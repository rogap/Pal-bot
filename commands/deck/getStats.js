...
module.exports = async (userId, nameOrId) => {
    try {
        ...
        const responseGetplayerloadouts = await _local.hirez.getplayerloadouts(playerId, 1)
        if (!responseGetplayerloadouts.status) throw responseGetplayerloadouts

        const getplayerloadouts = responseGetplayerloadouts.data

        return {
            getplayerloadouts: {
                lastUpdate: getplayerloadouts.lastUpdate,
                data: getplayerloadouts,
                old: responseGetplayerloadouts.old
            },
            playerId,
            playerName,
            status: true
