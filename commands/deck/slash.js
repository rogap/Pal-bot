...
module.exports = async (interaction, settings, command) => {
    try {
        ...
        const userNameOrId = interaction.options.getString('nickname')
        const numberOpt = interaction.options.getString('number')
        ...
        const exe = await command.execute(userId, settings, command, userNameOrId, champion, numberOpt)
