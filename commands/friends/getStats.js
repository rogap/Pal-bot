...
module.exports = async (userId, nameOrId) => {
    try {
        ...
        const responseGetfriends = await _local.hirez.getfriends(playerId)
        if (!responseGetfriends.status) throw responseGetfriends
        const getfriends = responseGetfriends.data

        return {
            getfriends: {
                lastUpdate: responseGetfriends.lastUpdate,
                data: getfriends
            },
            playerId,
            playerName,
